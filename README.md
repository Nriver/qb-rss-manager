...
填写`关键字`时会触发自动填充机制, 程序会依据配置尝试自动填充订阅地址.
填写`保存路径`时会触发自动填充机制, 程序会解析保存路径, 尽量解析出所有的数据. 推荐的保存路径格式举例 `Z:\Anime\XXXXX (2023)\Season 1`. 程序可以解析出的数据 `XXXXX` 对应 `{series_name}`, 四位数的年份 `2023` 对应 `{year}`

全局配置中的 `keyword_default` 可以配置默认的订阅地址, 默认为 `{series_name}`, 参考配置值举例 `A组 {series_name} | B组 {series_name}`.
全局配置中的 `rss_default` 可以配置默认的订阅地址, 该配置可以被分组数据中的 `rss_override` 配置给覆盖.
